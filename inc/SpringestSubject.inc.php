...
class SpringestSubject {
    ...
    /**
    * Get a SpringestSubject instance
    *
    * @param int $id The local ID of a subject, if set the instance is filled with data from the database, otherwise an empty instance is created
    * @return none
    */
    ...
    /**
    * Load the current subject instance with data from the database. The $trainings property is loaded separately for better performance since it's not always needed 
    *
    * @param string $field "id" to load the object by subject ID, "slug" to load it by slug/shortname
    * @param int|string $identifier The ID or short name identifier by which to retrieve the subject from the database
    * @return none
    */
    function loadBy($field, $identifier) {
        ...
        switch($field) {
            case "id" :
                $query = $wpdb->prepare("SELECT * FROM $wpdb->springest_subjects WHERE id = %d", $identifier);
            break;
            case "slug" : 
                $query = $wpdb->prepare("SELECT * FROM $wpdb->springest_subjects WHERE slug = %s", $identifier);
            break;
        }
        ...
        if($result) {
            ...
            $this->remoteId = $result->remote_id;
    ...
    */
    function loadTrainings() {
        ...
        $trainings = $api->getTrainings($this->remoteId, $offset);
    ...
    */
    function save() {
       global $wpdb;
       $insert = $wpdb->query(
           $wpdb->prepare("INSERT INTO $wpdb->springest_subjects (group_id, remote_id, name, slug, position) VALUES(%d, %d, %s, %s, %d)", $this->groupId, $this->remoteId, $this->name, $this->slug, $this->position)
       );
    }
